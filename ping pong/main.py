...
#mediapipe- hand landmark karta hai it has specified points for images(check google for hand landmark)

capture = cv.VideoCapture(0) #0 means self comp camera and 1 is for external camera
detector = HandDetector(detectionCon=0.8, maxHands=2) # idar detectioncon means agar voh 80% confirm hoga then it will condider those hands

capture.set(3,1280)  #height(4)=720 and width(3)=1280 3,4 representation hai
capture.set(4,720)

background = cv.imread('Resources\Background.png')  #background variable mein path store hoga and it will give a matrix when printed

ball = cv.imread('Resources\Ball.png', cv.IMREAD_UNCHANGED)
bat1 = cv.imread('Resources\Bat1.png', cv.IMREAD_UNCHANGED)    #imread_unchanged keeps the images unchanged  bcoz imread does some preprocessing on its own
bat2 = cv.imread('Resources\Bat2.png', cv.IMREAD_UNCHANGED)
gameOver = cv.imread('Resources\GameOver.png')

position = [100,100]  #specifies initial position of ball
isOver = False
speedX , speedY = 10, 10
score = [0, 0]
